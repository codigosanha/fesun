...
class Usuarios extends CI_Controller {
	...
	public function store(){
		...
		if ($this->Usuarios_model->save($data)) {
			$this->session->set_flashdata("success", "Los datos del usuario se guardo correctamente");
			redirect(base_url()."administrador/usuarios");
		}else{
			$this->session->set_flashdata("error", "Los datos del usuario no se guardaron");
	...
	public function saveFinca(){
		...
		if ($this->Usuarios_model->saveFincas($data)) {
			$data  = array(
				'fincas' => $this->Usuarios_model->getFincas($idUsuario), 
			);
			$this->load->view("admin/usuarios/fincas",$data);
		}else{
			echo "0";
		}
	}
	...
	public function getFincas(){
		$id = $this->input->post("id");
		$data  = array(
			'fincas' => $this->Usuarios_model->getFincas($id), 
		);
		$this->load->view("admin/usuarios/fincas",$data);
	}
